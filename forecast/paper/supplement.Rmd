...
```{r bootstrap-loading}
fcasts_booted <- readRDS(here("data","results_honest_bootstrapped.RDS")) %>%
  process_res_cases(actuals) %>%
  filter(forecaster != "gs_inherit")
if (params$fd_casefloor >= 0) {
...
con_tab <- hot_udf %>% 
  ...
  mutate(pred = value > cutoff) %>%
  group_by(forecaster, udf) %>%
  summarise(m = mean(pred != actual) - .5) %>%
  ungroup()
...
llike_tab <- hot_udf %>%
  ...
  mutate( # kill prob 0-1 predictions
    ...
    llike = (actual == 1) * log(value) + (actual == 0) * log(1 - value),
    nulldev = (actual == 1) * log(.5) + (actual == 0) * log(1 - .5)) %>%
  group_by(forecaster, udf) %>%
  summarise(m = mean(llike) - mean(nulldev)) %>%
  ungroup()
