...
```{r cumulative-mean, fig.cap="This figure shows the cumulative sum of WIS for each forecaster divided by the cumulative sum of WIS for the baseline model. The shaded background shows total U.S. cases as reported by JHU-CSSE." }
...
us_cases <- covidcast::covidcast_signal(
  "jhu-csse", "confirmed_7dav_incidence_num", "2020-03-01", "2021-01-01",
...
ggplot(cumulatives %>% filter(forecast_date < "2021-01-01"), 
  ...
  annotate(x = us_cases$time_value, ymax = us_cases$value, ymin = -Inf,
           geom = "ribbon", alpha = .1) +
  geom_hline(yintercept = 1) +
  xlab("forecast date") +
  scale_color_manual(values = fcast_colors, guide = guide_legend(nrow = 1)) +
