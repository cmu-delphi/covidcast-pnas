...
\significancestatement{Forecasting is a critical element of the public health
  ...
  internet% Capitalization is apparently
  % in question: https://en.wikipedia.org/wiki/Capitalization_of_Internet
  search activity all provide a nontrivial boost in accuracy, though
...
\equalauthors{\textsuperscript{2}J.B., A.G., and A.H.\ contributed equally to
  this work.}  
...
\begin{abstract}
Reliable short-term, location-specific forecasts of a pandemic are a
key ingredient in effective public health decision making. Throughout the
COVID-19 pandemic, a plethora of real-time data signals were
collected, curated, and made publicly available by our team and data
partners, providing multiple views of the pandemic's progress. The
goal of this work is to measure the practical utility of these
auxiliary indicators from a forecasting perspective. We focus on five
indicators, derived from medical insurance claims data, web search
queries, and survey responses. For each indicator, we ask whether its
inclusion in a simple model leads to improved forecasting
performance relative to a similar model excluding it. Performance is considered across two tasks
at a forecasting horizon of up to about one month:
probabilistic forecasting of seven-day average case rates and binary
prediction of ``hotspots.'' Since the values of indicators are
commonly revised over time, we take special care to ensure that the data provided
to a forecaster is the version that would have been available at the
time that the forecast was made. We demonstrate how less careful
backtesting can lead to misleading retrospective evaluations. Our analysis
suggests consistent, although 
modest, gains in forecasting performance by using these indicators,
and further investigation suggests that these gains are related to periods when the
auxiliary signals are acting as ``leading 
indicators'' of case rates. 
\attn{Evaluating forecasters thoroughly and fairly is an important way that
we can learn from this pandemic to do better in the next. 251 :(}
...
JHU CSSE \cite{Dong:2020}, which, like the auxiliary indicators that we use to supplement the
basic purely autoregressive models, is accessible through the COVIDcast API
\cite{CovidcastAPI}.
...
\item Google Search Trends for anosmia and ageusia (Google-AA): A measure of 
  Google search volume for queries relating to anosmia or
...
\cite{GoogleSymptoms}, and see \cite{Klopfen:2020, Vaira:2020} for justification of the
relevance of anosmia or ageusia to COVID-19 infection.
...
useful in predicting case activity rather than hospital demand. The \attn{former?} is
...
\begin{figure}[tb!]
  ...
  \caption{This figure displays revision behavior for two signals in the HRR
    containing New York 
    City.  Each colored line corresponds to the data as reported on a particular 
...
after initial publication.  Compared to the revision
...
in very nontrivial revisions.  See
...
uninteresting artifacts like day-of-the-week effects in the series.  Note
that we can equivalently view this (equivalent up to a constant
...
At prediction time, in order to avoid crossing violations (that is,
for two levels $\tau' > \tau$,
the predicted quantile at level $\tau$ exceeds the predicted quantile at level
...
\caption{This figure displays a forecast from an autoregressive model for the
  HRR containing
  New York City made  
...
(first term in each summand) and an ``under/overprediction'' component (second term
in each summand).  The second form given in \eqref{eq:wis_quantiles} is
especially noteworthy in our current study because it reveals WIS to be the same as the
...
the logistic models in \eqref{eq:hotspot_ar} and \eqref{eq:hotspot_ar_x} using the 
...
models of the form \eqref{eq:forecast_ar_x} or \eqref{eq:hotspot_ar_x} (five from
each of the forecasting and hotspot prediction tasks). We begin with a summary
...
\item In a more detailed analysis of forecasting performance, we find that the
  indicator models tend to be
  better than AR when case rates
  are flat or decreasing (most notable in CHNG-COVID and CTIS-CLI-in-community),
  but they are
  worse than AR when case rates are increasing (this
  is most notable in CHNG-CLI and DV-CLI).
...
\item In this same analysis, when an indicator model performs better than AR in
  ...
  given location, its behavior mimics that of future case rate behavior).  On
...
This makes sense since they are both claims-based indicators supposedly measuring
the same thing.  However, DV-CLI outperforms CHNG-CLI on vintage
...
fact that CHNG-CLI suffers from spatially heterogeneous volatility due to
backfill.  The evidence for this explanation is twofold: (1) the CHNG-CLI model benefits from a
more robust method of aggregating WIS (geometric mean; shown in the supplement);
and (2) when we train and make predictions on finalized data, it handily beats
AR, on par with the best-performing models (also shown in the 
supplement).
...
the baseline (in this context, a classifier based on random guessing, which
...
$X_{\ell,t}$ in the each indicator models
%in \eqref{eq:forecast_ar_x} \eqref{eq:hotspot_ar_x}
replaced with suitable random noise (bootstrap samples
...
models
% (with random noise as auxiliary features)
offered essentially no
...
samples and only 3--6 features). [DJM: and that proper CV requires longer
training windows, exacerbating the nonstationarity, a scenario in which its
behavior is poorly understood.]

% \begin{figure}[t]
% \includegraphics[width=\columnwidth]{fig/hot-1.pdf}
% \caption{Area under the curve for each hotspot prediction model, calculated over
...
% Essentially, during downswings, improved performance is strongly correlated
% with predicing larger declines than the AR model. The AR model tends to
% overpredict relative to the decline, but adding indicators tends to diminish
% this effect. During upswings, the AR model tends to underpredict, but when the
% addition of an indicator improves performance (a more rare occurrence) it is 
% rarely due to higher median predictions.

% RJT: Commented out the last paragraph for now, because this is kind of a tough
% thing to explain succintly. Can revisit if we really want to include it here,
% or just let them find it in the supplement?
% DJM: Agreed. In the supplement.
...
surprisingly, this relationship turns out to be strongest in down periods
...
\acknow{We thank Matthew Biggerstaff, Logan Brooks, Johannes Bracher, Michael 
  ...
  enlightening conversations about forecast evaluation. The research was
  supported by Centers for Disease Control and Prevention (CDC) grant
  U01IP001121; gifts from Facebook, Google, McCune, and Optum; the National
  Science Foundation Graduate Research Fellowship Program (NSF GRFP) award
  DGE175016; the Canadian Statistical Sciences Institute; and the National
  Sciences and Engineering Research Council of Canada (NSERC) RGPIN-2021-02618.} 
