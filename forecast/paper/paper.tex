...
Along the lines of regularization, it is also worth noting that the use of
...
training samples is quite large compared to the number of features (6,426
training samples and only 3--6 features).  
...
\begin{figure}[t]
  ...
  \caption{Histogram of the difference in WIS for the AR model and that for each
    indicator model, stratified by up, down, or flat periods, measured in terms
    of case activity. (Larger values are better for the indicator model.) The
    y-axis is on the log scale to emphasize tail behavior.} 
...
tests reported by public health in a particular location, we would see an
increase in insurance claims for COVID-19-related medical visits.  However, it
may well be the case that this leading behavior is different during different
periods. In fact, we find that the ``leadingness'' of an indicator (degree to
which it leads) tends to be more pronounced during down and flat periods than up
periods, a plausible explanation for the decreased performance noted above.   

In order to measure the leadingness on af indicator, we examine its cross
correlations to case rates over a short time window

 the cross
correlation function. Essentially this examines the Pearson correlation between 
