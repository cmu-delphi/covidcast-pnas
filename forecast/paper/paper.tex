...
---which is that 
  defining features from lagged indicator values in the  models in
  \eqref{eq:forecast_ar_x}, \eqref{eq:hotspot_ar_x} simply provides a kind of
  implicit regularization on top of the basic AR model (so the benefits have
  nothing to do with the actual indicator values)---


To test the ``implicit regularization'' hypothesis (which we explain
  precisely later), we reran all of the prediction experiments but with
  $X_{\ell,t}$ in each indicator model replaced by suitable random noise.  This 
  resulted in vastly different (worse) performance compared to the indicator
  models fit with actual indicator values, and gave no improvement over the 
  pure AR model, in both forecasting and hotspot prediction, providing a strong
  rejection (informally speaking) of the implicit regularization hypothesis.  

\subsection{Performance in Up, Down, Flat Periods}

\subsection{Leading-ness or Lagging-ness}
...
A Primary 
B Bootstrap
C up/down/flat (Dan's first histogram) --> table in appendix
D leading/lagging 
