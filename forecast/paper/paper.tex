...
nonlinearities.
% Addition in response to Reviewer 1
...
% Added in response to Reviewer 2
...
Our internal evaluations suggest that the same is true (at least, if not to a
greater extent) for case forecasting. 
...
% Removed: this is no longer true unfortunately.
% because few hotspots
...
the main evaluation metric used in the COVID-19 Forecast Hub. More broadly, the
specific form of WIS used here is a standard metric in the forecasting community
for evaluating quantile-based probabilistic forecasts, just as mean squared
forecast error is standard for point forecasts. 
