...
\significancestatement{Forecasting is a key component in the public health
  response to any fast-moving epidemic or pandemic.  Forecasting the future
  ...
  literature---is a basic and widely-used method, which can be especially 
  effective in noisy data settings.  In this paper we ask whether including
  ...
  that auxiliary indicators based on de-identified medical insurance claims,
  self-reported symptoms through online surveys, and COVID-related Internet
  search activity all provide a nontrivial boost in accuracy, though their
  utility generally depends the phase of the pandemic that we are in at
  prediction time (upswing, downswing, or flat period).
}
...
%   \caption{Average WIS for each forecast model, over all forecast dates and all
