...
pattern in the medical claims signals, which are appears to be much more
systematic in nature, the revision pattern in the case rates signal can be
highly erratic, resulting in big spikes as reporting backlogs are cleared,
changes in case definitions are made, other corrections are applied, etc.  

TODO Are the above problems mentioned about case rates really to do with vintage 
data, or would they still exist in finalized data?

TODO Should we discuss the use of ``bad'' case rates as ground truth?? 

TODO Discuss Google-AA.  We're using all finalized data, but explain why we're
OK with this ... (they never backfill).
...
The first task we consider---\textit{forecasting}---is to predict the HRR-level
COVID-19 case incidence rates, averaged over some 7-day time period in the
future (the average number of new daily reported cases per 100,000 people, over
the 7-day period).  The second task---\textit{hotspot prediction}---is to
predict a binary variable defined in terms of relative changes of the
aforementioned target variable. Why define the target variable here via 7-day 
averaging? The short answer is robustness: averaging stabilizes the case time
series, and accounts for uninteresting artifacts like weekend-weekday
differences in the series.  Note that we can of course equivalently view this
(equivalent up to a constant factor) as predicting HRR-level COVID-19 case
incidence rates \textit{summed} over some 7-day period in the future (the total
number of new reported cases per 100,000 people, over the 7-day period), or
predicting a binary variable defined in terms of its relative changes.

To fix notation, let $Y_{\ell,t}$ denote the 7-day trailing average of COVID-19
case incidence rates in location (HRR) $\ell$ and at time (day) $t$.  In 
forecasting, a time $t$, we seek to predict $Y_{\ell,t+a}$ for each ``ahead''
value $a=7,\ldots,21$. Meanwhile, in hotspot prediction, we seek to predict
$Z_{\ell,t+a}$, a binary indicator defined in terms of the relative difference 
$(Y_{\ell,t+a}-Y_{\ell,t+a-7}) / Y_{\ell,t+a-7}$, again for each
$a=7,\ldots,21$.   
...
prediction models are always fit to data across all HRRs (i.e., the coefficients
in the models do not account for location-specific effects).  

In our analysis, we let the prediction date $t$ run over each day in between
early June and December 31, 2020.  The precise start date differs for
...
the survey was first launched). The end date was chosen with a consideration
again to align both tasks as best as possible, and because few hotspots exist
post December 31, 2020, due to the general and gradual decline of the pandemic
in 2021.

\subsubsection{Task 1: Forecasting}
...
\end{equation}
To estimate the coefficients, we use \textit{quantile regression}
\cite{Koenker:1978, Koenker:2005, Koenker:2006} fit separately for each $\tau$,
using data from all 306 HRRs, and within each HRR, using the most recent 21 days
of training data.  To be clear, this gives 6,426 training samples for each
...
7 and 14 days ago) for the auxiliary signals as we do for case rates. Estimation
proceeds just as before: we use the same 7 quantile levels as in
\eqref{eq:quantile_levels}, and again use quantile regression separately for
each $\tau$, fit using data from all 306 HRRs and a trailing window of 21 days
of training data.
...
\caption{Forecasts and observed cases for HRR x. TODO 
  \attn{Create a figure here for 1 HRR. Show the fan for the quantiles.}}  
...
compared to the preceding week.  When this occurs, we call location $\ell$ a
