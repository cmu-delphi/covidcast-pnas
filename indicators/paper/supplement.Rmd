...
```{r state-trend-grids-1, fig.cap="Trends of cases, CHNG-CLI, CHNG-COVID, and CTIS-CLI-in-community for U.S. states and territories. Cases are displayed on the rate scale: counts per 100,000 people. Other signals are scaled to have the same global maximum across all counties and times, so they can be presented in the same range. (Part 1 of 4.)", fig.height=8}
...
```{r county-trend-grids-1, fig.cap="Trends of cases, CHNG-CLI, CHNG-COVID, and CTIS-CLI-in-community for the 50 most populous U.S. counties. Cases are displayed on the rate scale: counts per 100,000 people. Other signals are scaled to have the same global maximum across all counties and times, so they can be presented in the same range. (Part 1 of 4.)", fig.height=8}
g <- get_trend_grid(
  county_trends,
  function(fipsen) {
