...
\begin{abstract} % 250 words max, one paragraph
  The COVID-19 pandemic in the United States presented enormous data challenges.
  Policymakers, epidemiological modelers, and health researchers all require
  up-to-date data on the pandemic and relevant public behavior, ideally at fine
  spatial and temporal resolution. We present the COVIDcast API, which provides
  an open database of both traditional public health surveillance signals (such
  as cases, deaths, and hospitalizations) and unique indicators of COVID-19
  activity, such as signals extracted from de-identified medical claims data,
  massive online surveys, cell phone mobility data, and Internet search trends.
  These signals are available at a fine geographic resolution (mostly at the
  county level) and are updated daily. Unlike most public health databases, the
  COVIDcast API also tracks all revisions to historical data, allowing modelers
  to account for the frequent revisions and backfill that are common for many
  public health data sources. All data is available in a common format through
  an API and software packages. We describe the data and formats, and present
  examples demonstrating that the non-traditional data sources present in the
  COVIDcast API present information relevant to tracking COVID activity,
  augmenting standard public health reporting and enabling research.
\end{abstract}
...
Survey; our companion paper on this survey gives more a detailed view of its
...
revision data is rarely available in standardized format from other sources. 
...
or geographic differences in market share for medical claims data. Claims data
can also be subject to biases during major national holidays and other events
that change health-seeking behavior. Characterizing these biases will be
important for future research and operational systems that use these
signals. Several data sources are also subject to extensive revision and
...
features of the COVIDcast API will help facilitate this work, which will be
vital to advancing pandemic modeling and preparedness.  
