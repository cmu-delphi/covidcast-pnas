...
anosmia <- data %>%
  mutate(date = as.Date(StartDatetime),
         anosmia = is_selected(B2, "13"),
         state = toupper(name_to_abbr(state_fips_to_name(fips)))) %>%
  group_by(date, state) %>%
  summarize(anosmia = weighted.mean(anosmia, weight, na.rm = TRUE)) %>%
  select(time_value = date, value = anosmia, geo_value = state) %>%
  as.covidcast_signal(signal = "anosmia")
...
signals <- append(signals, anosmia)
